...
#
# This script shows how to perform arithmetic operations like addition and subtraction on images.
# It also expands over the differences in Numpy and OpenCV based operations
#
#################################################################################################
...
# To understand above statement, consider if a pixel value of 100 is added to 200.
# The result is 300. But pixels cannot be larger than 255 in value.
# So OpenCV will clip the result of an addition operation to 255 if values go beyond it.
# Likewise, for a subtraction operation, say 100 is subtracted from 50 resulting in -50.
# OpenCV will clip the result at 0 and not give -50 since 0 is the lower bound for a pixel value.
# It is better to use OpenCV for arithmetic to avoid confusion.
addition = cv2.add(np.uint8([200]), np.uint8([100]))
subtraction = cv2.subtract(np.uint8([50]), np.uint8([100]))
print(f"Adding 100 and 200 using OpenCV = \n{addition}")
print(f"Subtracting 100 from 50 using OpenCV = \n{subtraction}")
...
# So, 200 + 100 = 255 and then start counting from 0 again.
# This gives us value of 44 (since 255 in initial count and then restarting count from 0)
...
ap.add_argument("-i", "--image", type=str, default=DATA_DIR / "images/sun.jpg", help="Input image to crop")
